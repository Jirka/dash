...
import cz.vutbr.fit.dashapp.segmenation.util.bottomup.BUConnectSmallRegionsUtil;
import cz.vutbr.fit.dashapp.segmenation.util.bottomup.BottomUp1;
import cz.vutbr.fit.dashapp.segmenation.util.bottomup.BottomUp1.BottomUp1Result;
import cz.vutbr.fit.dashapp.segmenation.util.bottomup.BottomUp2;
import cz.vutbr.fit.dashapp.segmenation.util.bottomup.BottomUp2.BottomUp2Result;
...
public class BottomUpRefactorized extends AbstractSegmentationAlgorithm implements ISegmentationAlgorithm {
	...
	public BottomUpRefactorized(DebugMode debugMode) {
		super(debugMode);
	}
	...
	public Dashboard processImage(BufferedImage image) {
		...
		debug("bottom up 1", GrayMatrix.printMatrixToImage(null, result1.matrix));
		
		// version 2
		// TODO bottomUp2 contains some unclear code which should be replaced
		BottomUp2Result result2 = BottomUp2.bottomUp2(matrix);
		debug("bottom up 2", GrayMatrix.printMatrixToImage(null, result2.matrix));
		
		// combination
		int[][] finalCombination = BUBasicUtil.combineResults(result1.matrix, result2.matrix);
		debug("combination", GrayMatrix.printMatrixToImage(null, finalCombination));
