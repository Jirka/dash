package cz.vutbr.fit.dashapp.eval.analysis.heatmap.old;
...
import cz.vutbr.fit.dashapp.eval.analysis.heatmap.AbstractHeatMapAnalysis;
...
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.RasterRatioCalculator.HSBRatioCalculator_sb;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.RasterRatioCalculator.HSBRatioCalculator_sb05;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.RasterRatioCalculator.HSBRatioCalculator_sb05_max1;
...
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.RasterRatioCalculator.HSBRatioCalculator_sblog;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.RasterRatioCalculator.HSBRatioCalculator_sblog_x;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.RasterRatioCalculator.HSLRatioCalculator;
...
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoNormRasterDensity;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoNormRasterDensity_X;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoNormRasterUnity;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoNormRasterUnity_X;
...
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoRasterDensity_X;
...
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoRasterSymmetry2;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoRasterUnity;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.NgoRasterUnity_X;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterBalance;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterDensity;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterDensity_X;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterEquilibrium;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterFinalDensity;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterFinalDensity_X;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterSymmetry;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterSymmetry2;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterUnity;
//import cz.vutbr.fit.dashapp.eval.metric.widget.raster.ngo.my.MyRasterUnity_X;
...
public class WidgetMetricAnalysis extends AbstractHeatMapAnalysis {
	...
	Map<WorkspaceFolder, Map<IMetric, MeanStatistics[]>> meanValues;
	Map<WorkspaceFolder, Map<IMetric, MeanStatistics[]>> meanValuesFiltered;
	Map<WorkspaceFolder, Map<IMetric, MeanStatistics[]>> meanValuesCrop;
	...
	public void processFolder(WorkspaceFolder actWorkspaceFolder) {
		DashboardCollection actDashboards = getDashboardCollection(actWorkspaceFolder, "x.*");
		...
		if(crop || filtered) {
			DashboardCollection cropDashboard = DashAppUtils.makeDashboardCollection(
					actWorkspaceFolder.getChildren(
							DashboardFile.class, actWorkspaceFolder.getFileName() + "_body", false
					));
	...
	private Map<IMetric, MeanStatistics[]> analyzeDashboards(WorkspaceFolder actWorkspaceFolder, DashboardCollection actDashboards) {
		//printDashboards(actWorkspaceFolder, actDashboards, label);
		return calculateMetrics(actWorkspaceFolder, actDashboards);
	}

	private Map<IMetric, MeanStatistics[]> calculateMetrics(WorkspaceFolder actWorkspaceFolder, DashboardCollection actDashboards) {
		Map<IMetric, MetricResultsCollection> resultsCollection = MetricCalculator.measure(
				actDashboards,
				metrics,
				GEType.ALL_TYPES
		);
		Map<IMetric, MeanStatistics[]> stats = MetricCalculator.statistics(resultsCollection, FILTER_EXTREME_ITEMS);
		return stats;
	}
	...
	public void sumarizeFolders(WorkspaceFolder actWorkspaceFolder, List<WorkspaceFolder> analyzedFolders) {
		for (IMetric metric : metrics) {
			...
			for (WorkspaceFolder workspaceFolder : analyzedFolders) {
				...
				MeanStatistics[] stats = meanValues.get(workspaceFolder).get(metric);
				sb.append(getFirstStat(stats));
				if(filtered) {
					MeanStatistics[] statsFiltered = meanValuesFiltered.get(workspaceFolder).get(metric);
					sb.append(getFirstStat(statsFiltered));
				}
				if(crop) {
					MeanStatistics[] statsCrop = meanValuesCrop.get(workspaceFolder).get(metric);
	...
	private String getFirstStat(MeanStatistics[] stats) {
	...
	@SuppressWarnings("unused")
	private void formatStats(MeanStatistics[] stats, StringBuffer sb) {
		for (MeanStatistics stat : stats) {
